...
const Contact = () => {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ marginTop: "150px" }}>
      <div className="container-lg grid gap-6 lg:gap-7 3xl:gap-10">
        <div className="grid w-full gap-x-4 gap-y-6 md:grid-cols-2  3xl:gap-10">
          <InputGroup name="name" label={"Full Name*"}>
            <Input type="text" name="name" required className="border-[#DEE3E9]" />
          </InputGroup>

          <InputGroup name="company" label={"Your Company*"}>
            <Input type="text" name="company" required className="border-[#DEE3E9]" />
          </InputGroup>

          <InputGroup name="email" label={"Business Email*"}>
            <Input type="text" name="email" required className="border-[#DEE3E9]" />
          </InputGroup>

          <InputGroup name="phone" label={"Phone*"}>
            <Input type="text" name="phone" required className="border-[#DEE3E9]" />
          </InputGroup>
        </div>

        <InputGroup name="message" label={"Send Us a Message (Optional)"}>
          <Textarea type="text" name="message" className="rounded-[24px] border-[#DEE3E9]" />
        </InputGroup>

        <Button size="lg" disabled={state.submitting} className=" ml-auto w-fit px-10 py-7" variant="default">
          {state.submitting && (
